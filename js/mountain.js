...
(function () {
  ...
  /* contour-line shader: glowing iso-elevation lines over a dark mesh */
  var mat = new THREE.ShaderMaterial({
    transparent: true,
    uniforms: {
      uSteel: { value: new THREE.Color(0x3a8ec2) },
      uHigh: { value: new THREE.Color(0xc5dff0) },
      uGold: { value: new THREE.Color(0xc9a84c) },
      uBase: { value: new THREE.Color(0x0a1220) },
      uMaxH: { value: H },
    ...
    fragmentShader: [
      'varying float vH;',
      'varying float vR;',
      'uniform vec3 uSteel; uniform vec3 uHigh; uniform vec3 uGold; uniform vec3 uBase;',
      'uniform float uMaxH; uniform float uRadius;',
      'void main() {',
      '  float hn = clamp(vH / uMaxH, 0.0, 1.0);',
      '  /* iso contour lines every (uMaxH / 26) of elevation */',
      '  float bands = vH / uMaxH * 26.0;',
      '  float d = fwidth(bands);',
      '  float line = 1.0 - smoothstep(0.0, d * 1.6, abs(fract(bands) - 0.5) - (0.5 - d * 1.6));',
      '  /* line brightness climbs with altitude; snowline goes pale */',
      '  vec3 lineCol = mix(uSteel, uHigh, smoothstep(0.55, 0.95, hn));',
      '  lineCol = mix(lineCol, uGold, smoothstep(0.93, 1.0, hn) * 0.55);',
      '  float lineAlpha = line * (0.22 + 0.78 * hn);',
      '  /* dark faceted base, slightly lifted by height */',
      '  vec3 base = mix(uBase, uSteel * 0.35, hn * 0.35);',
      '  float edge = 1.0 - smoothstep(uRadius * 0.55, uRadius * 0.98, vR);',
      '  vec3 col = base + lineCol * lineAlpha;',
      '  float alpha = max(lineAlpha, 0.82) * edge;',
